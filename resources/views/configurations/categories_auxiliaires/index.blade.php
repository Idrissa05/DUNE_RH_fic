        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="exampleModalLabel1">Nouvelle catégorie Auxiliaire</h4>
            ...
            <table class="table table-bordered text-center" id="myTable">
                ...
                @foreach($categories as $categroy)
                    <tr>
                        ...
                        @can('ACTIONS_CONFIGURATION')
                        <td>
                            <button id="category{{ $categroy->id }}" data-name="{{ $categroy->name }}" data-route="{{ route('categoryAuxiliaire.update', $categroy) }}"
                                    onclick="updateCategory({{ $categroy->id }})" class="btn btn-sm btn-outline-warning">
                                <i class="mdi mdi-18px mdi-pencil"></i>
                            </button>

                            <form action="{{ route('categoryAuxiliaire.destroy', $categroy) }}" id="del{{ $categroy->id }}" style="display: inline-block;" method="post">
                                @method('DELETE')
                                @csrf
