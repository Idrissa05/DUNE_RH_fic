...
@section('content')
    ...
    <h3 class="m-b-0 text-white text-center bg-primary">Les positions</h3>
            ...
            <table class="table table-bordered text-center" id="myTable">
                ...
                @foreach($positions as $position)
                    <tr>
                        ...
                        @can('ACTIONS_CONFIGURATION')
                        <td>
                            <button id="position{{ $position->id }}"
                                    data-name="{{ $position->name }}"
                                    data-route="{{ route('position.update', $position) }}"
                                    onclick="updatePosition({{ $position->id }})" class="btn btn-sm btn-outline-warning">
                                <i class="mdi mdi-18px mdi-pencil"></i>
                            </button>


                            <form action="{{ route('position.destroy', $position) }}" id="del{{ $position->id }}" style="display: inline-block;" method="post">
                                @method('DELETE')
                                @csrf
                                <button class="btn btn-outline-danger btn-sm" type="button"
                                        onclick="myHelpers.deleteConfirmation('{{ 'del'. $position->id }}')"
