        ...
        return DataTables::of(Dece::with(['agent' => function($query) { return $query->withTrashed(); }])->orderBy('created_at', 'desc')->get())
            ...
            ->addColumn('actions', function ($dece){
                return '<div class="btn-group">
                    <a title="editer" href="'.route('deces.edit', $dece).'" class="btn btn-outline-warning btn-sm mr-2"><i class="mdi mdi-account-edit mdi-18px"></i></a>
                    <form action="'.route("deces.destroy", $dece).'" id="del'.$dece->id.'" style="display: inline-block;" method="post">
                                '.method_field('DELETE').'
                                '.csrf_field().'
